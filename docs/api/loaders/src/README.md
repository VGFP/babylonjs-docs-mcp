[**@babylonjs/root v1.0.0**](../../README.md)

***

[@babylonjs/root](../../README.md) / loaders/src

# loaders/src

## Namespaces

- [GLTF1](namespaces/GLTF1/README.md)
- [GLTF2](namespaces/GLTF2/README.md)

## Enumerations

- [GLTFLoaderAnimationStartMode](enumerations/GLTFLoaderAnimationStartMode.md)
- [GLTFLoaderCoordinateSystemMode](enumerations/GLTFLoaderCoordinateSystemMode.md)
- [GLTFLoaderState](enumerations/GLTFLoaderState.md)

## Classes

- [BVHFileLoader](classes/BVHFileLoader.md)
- [FBXFileLoader](classes/FBXFileLoader.md)
- [GaussianSplattingStream](classes/GaussianSplattingStream.md)
- [GLTFFileLoader](classes/GLTFFileLoader.md)
- [GLTFValidation](classes/GLTFValidation.md)
- [MTLFileLoader](classes/MTLFileLoader.md)
- [OBJFileLoader](classes/OBJFileLoader.md)
- [SolidParser](classes/SolidParser.md)
- [SPLATFileLoader](classes/SPLATFileLoader.md)
- [STLFileLoader](classes/STLFileLoader.md)

## Interfaces

- [FBXFileLoaderOptions](interfaces/FBXFileLoaderOptions.md)
- [GLTFLoaderExtensionOptions](interfaces/GLTFLoaderExtensionOptions.md)
- [GLTFLoaderOptions](interfaces/GLTFLoaderOptions.md)
- [IGaussianSplattingStreamOptions](interfaces/IGaussianSplattingStreamOptions.md)
- [IGLTFLoaderData](interfaces/IGLTFLoaderData.md)
- [IGLTFLoaderExtension](interfaces/IGLTFLoaderExtension.md)
- [IGLTFValidationConfiguration](interfaces/IGLTFValidationConfiguration.md)
- [ISOGLODMetadata](interfaces/ISOGLODMetadata.md)

## Type Aliases

- [FBXNormalMapCoordinateSystem](type-aliases/FBXNormalMapCoordinateSystem.md)
- [GaussianSplattingStreamDebugLodSource](type-aliases/GaussianSplattingStreamDebugLodSource.md)
- [MaterialVariantsController](type-aliases/MaterialVariantsController.md)
- [OBJLoadingOptions](type-aliases/OBJLoadingOptions.md)
- [SPLATLoadingOptions](type-aliases/SPLATLoadingOptions.md)

## Variables

- [FBXFileLoaderMetadata](variables/FBXFileLoaderMetadata.md)
- [GLTFLoaderDefaultOptions](variables/GLTFLoaderDefaultOptions.md)

## Functions

- [AddGaussianSplattingStreamPart](functions/AddGaussianSplattingStreamPart.md)
- [AddGaussianSplattingStreamPartAsync](functions/AddGaussianSplattingStreamPartAsync.md)
- [ReadBvh](functions/ReadBvh.md)
- [registerBuiltInLoaders](functions/registerBuiltInLoaders.md)
- [RegisterBVHFileLoader](functions/RegisterBVHFileLoader.md)
- [RegisterGLTFFileLoader](functions/RegisterGLTFFileLoader.md)
- [RegisterOBJFileLoader](functions/RegisterOBJFileLoader.md)
- [RegisterSPLATFileLoader](functions/RegisterSPLATFileLoader.md)
- [RegisterSTLFileLoader](functions/RegisterSTLFileLoader.md)
