#!/usr/bin/env bash
# Regenerate the bundled docs/ snapshot from upstream sources.
#
# Reads:
#   babylonjs-source/   (from clone-babylonjs-source.sh)  -> TypeDoc API reference
#   babylonjs-docs/     (from clone-babylonjs-docs.sh)    -> hand-written guides/examples
#
# Writes:
#   docs/api/           TypeDoc markdown for core/gui/loaders/materials/serializers
#   docs/examples/      Guides and examples copied from the Documentation repo
#   docs/README.md      Index page with stats
#
# The BabylonJS version is derived from THIS package's version
# (e.g. "9.13.0-mcp.1" -> "9.13.0").
set -euo pipefail
...
echo "=== Generating docs for BabylonJS $VERSION ==="
...
if [ ! -d "$BABYLON_SOURCE" ]; then
    echo "ERROR: $BABYLON_SOURCE not found. Run scripts/clone-babylonjs-source.sh first."
    exit 1
fi

# Rebuild docs/ from scratch - never merge stale files into an old snapshot.
rm -rf "$OUTPUT_DIR"
...
EXAMPLE_COUNT=$(find "$OUTPUT_DIR/examples" -name "*.md" | wc -l | tr -d ' ')
...
API_COUNT=$(find "$OUTPUT_DIR/api" -name "*.md" | wc -l | tr -d ' ')
...
echo ""
echo ">>> Next: run 'npm run lint && npm run smoke' to validate, then commit."
